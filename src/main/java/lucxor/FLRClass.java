...
class FLRClass {
  ...
  void evalTickMarks(int dataType) throws InterruptedException, ExecutionException {
    ...
    for (int i = 0; i < NMARKS; i++) {
      ...
      if (kernelResult > Constants.TINY_NUM) {
        ...
        res[i] = Constants.TINY_NUM;
      }

      pool.shutdown(); // bring down the threadpool
    }

    if (dataType == Constants.DECOY) {
      f0 = res;
    }

    if (dataType == Constants.REAL) {
      f1 = res;
    }
  ...
  public void setMinorMaps() {
    ...
    for (int iter = 0; iter < 2; iter++) {
      ...
      for (int i = 0, scoreListSize = scoreList.size(); i < scoreListSize; i++) {
        ...
        if (iter == 0) {
          minorMapG.put(i, pair);
        } else {
          minorMapL.put(i, pair);
        }
      }
    } // end loop over iter
  }
  ...
  public void performMinorization() {
    ...
    double[] deqPtr;
    THashMap<Integer, double[]> mapPtr;
